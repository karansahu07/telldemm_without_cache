...
export class LoadAllMembersPage implements OnInit {
      ...
      this.allUsers = [
        ...pfUsers
          .filter((u: any) => {
            // Exclude current user
            const userPhone = String(u.phoneNumber ?? '');
            if (currentUserPhone && userPhone === currentUserPhone)
              return false;
  ...
  async confirmSelection() {
    ...
    const communityLabel = this.communityName
      ? `"${this.communityName}"`
      : 'this community';
    const msg = `Members will also be added to the community ${communityLabel} and its announcement group.`;
    ...
    const alert = await this.alertCtrl.create({
      ...
      buttons: [
        { text: 'Cancel', role: 'cancel' },
        {
          text: 'Continue',
          handler: () => {
            // ✅ Save selected members to Firebase service
            this.firebaseChatService.setSelectedGroupMembers(selected);
            this.firebaseChatService.setCurrentCommunityContext({
              communityId: this.communityId,
              communityName: this.communityName,
            });

            console.log(
              '✅ Selected members saved to service:',
              selected.length
            );
  ...
  // ✅ NEW: Method to retrieve saved members on page load (optional)
  ionViewWillEnter() {
    // If you need to restore previously selected members when returning to this page
    const savedMembers = this.firebaseChatService.getSelectedGroupMembers();

    if (savedMembers && savedMembers.length > 0) {
      console.log(
        '📥 Restoring previously selected members:',
        savedMembers.length
      );

      // Mark users as selected based on saved members
      this.allUsers.forEach((user) => {
        const wasSelected = savedMembers.some(
          (saved) => saved.user_id === user.user_id
        );
        user.selected = wasSelected;
      });
  ...
  // ✅ NEW: Clear selection on page leave (optional)
  ionViewWillLeave() {
    // Optional: Clear saved members when leaving page
    // Uncomment if you want to clear on navigation away
