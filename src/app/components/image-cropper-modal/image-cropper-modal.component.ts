...
export class ImageCropperModalComponent implements OnInit, OnDestroy {
  ...
  private handleResize(clientX: number, clientY: number) {
    ...
    switch (this.resizeDirection) {
      case 'top-left':
        newWidth = Math.max(minSize, this.initialCropState.width - deltaX);
        newHeight = newWidth / this.aspectRatio;
        newX = this.initialCropState.x + this.initialCropState.width - newWidth;
        newY = this.initialCropState.y + this.initialCropState.height - newHeight;
        break;
    ...
    // Boundary checks with smoothing
    if (newX >= 0 && newY >= 0 && 
        newX + newWidth <= rect.width && 
        newY + newHeight <= rect.height) {
